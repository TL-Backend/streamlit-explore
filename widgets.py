...
def text_input_widget():
    st.markdown("**<u>Text Input</u>**", unsafe_allow_html=True)
    user_input = st.text_input("Enter your name")
    if user_input:
        st.write("Hello", user_input)

def slider_widget():
    st.markdown("**<u>Slider</u>**", unsafe_allow_html=True)
    age = st.slider("Select your age", 0, 100)
    if age:
        st.write("Your age is:", age)

def checkbox_widget():
    st.markdown("**<u>Check Box</u>**", unsafe_allow_html=True)
    agree = st.checkbox("I agree to the terms and conditions")
    if agree:
        st.write("Great! Let's proceed.")

def dropdown_widget():
    st.markdown("**<u>Dropdown</u>**", unsafe_allow_html=True)
    option = st.selectbox("Select a city", ["New York", "London", "Tokyo"])
    st.write("You selected:", option)

def button_widget():
    st.markdown("**<u>Button Action</u>**", unsafe_allow_html=True)
    if st.button("Click me"):
        st.write("Button clicked!")

def file_uploader_widget():
    st.markdown("**<u>File Uploader</u>**", unsafe_allow_html=True)
    uploaded_file = st.file_uploader("Choose a file")
    if uploaded_file:
        st.write("File uploaded:", uploaded_file.name)

def date_input_widget():
    st.markdown("**<u>Date Selector</u>**", unsafe_allow_html=True)
    selected_date = st.date_input("Select a date", datetime.date.today())
    st.write("Selected date:", selected_date)
    
def number_input_widget():
    st.markdown("**<u>Number Range</u>**", unsafe_allow_html=True)
...
def color_picker_widget():
    st.markdown("**<u>Color Picker</u>**", unsafe_allow_html=True)
    color = st.color_picker("Choose a color", "#ff6347")
    st.write("Selected color:", color)

def time_input_widget():
    st.markdown("**<u>Time Input </u>**", unsafe_allow_html=True)
